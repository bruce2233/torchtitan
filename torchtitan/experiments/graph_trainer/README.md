...
**Key features:**
- **Full train step graph capture** — `make_fx`-based `minimal_fx_tracer` traces forward + loss + backward into a single FX graph, without AOTAutograd partitioning, giving full visibility and control over the entire computation.
...
- **Tensor-granularity memory policy** — Each activation can independently be saved, recomputed, or CPU-offloaded, unlike module-level eager SAC. Different strategies mix freely within a single layer.
